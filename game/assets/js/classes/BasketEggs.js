// * Definições cesta de ovos:
class BasketEggs extends Sprite { // O ovo é uma extensão da classe Sprite, que é uma classe base para objetos que podem ser desenhados no canvas.
    constructor({ // O construtor da classe Enemy recebe um objeto com as propriedades necessárias para inicializar o ovo.
        ...
        // Propriedades cesta de ovos individual terá dentro de si:
        super({ imageSrc, frameRate, scale }) // Chama o construtor da classe pai (Sprite) para inicializar a imagem cesta de ovos
        this.position = position // Posição cesta de ovos (objeto com propriedades x e y)
        this.velocity = { // Velocidade cesta de ovos (objeto com propriedades x e y)
        ...
        this.hitbox = {
            position: {
                x: this.position.x, // Posição horizontal referente ao cesta de ovos
                y: this.position.y, // Posição vertical referente ao cesta de ovos
            },
            width: 10, // Largura do hitbox
            height: 10, // Altura do hitbox
        }

        this.animations = animations // Animações cesta de ovos (passadas como argumento)
        this.lastDirection = 'right'; // Define a última direção cesta de ovos como direita (padrão)

        for(let key in this.animations) { // Para cada animação cesta de ovos
        ...
        this.camerabox = { // Caixa da câmera cesta de ovos (para limitar a área de visão)
            position: {
                x: this.position.x, // Posição horizontal referente ao cesta de ovos
                y: this.position.y, // Posição vertical referente ao cesta de ovos
            },
    ...
    // * Método para alternar entre as animações cesta de ovos:
    switchSprite(key) { 
        if (this.image === this.animations[key].image || !this.loaded) return; // Se a imagem atual cesta de ovos for a mesma da animação correspondente, não faz nada
        
        this.currentFrame = 0; // Reseta o quadro atual para 0 (início da animação)
        this.image = this.animations[key].image; // Define a imagem atual cesta de ovos como a imagem da animação correspondente
        this.frameBuffer = this.animations[key].frameBuffer; // Define o buffer de quadros da animação correspondente
        this.frameRate = this.animations[key].frameRate; 
    }

    // * Método para atualizar a posição da caixa da câmera cesta de ovos:
    updateCamerabox() { 
        this.camerabox = { // Caixa da câmera cesta de ovos (para limitar a área de visão)
            position: {
                x: this.position.x - 50, // Posição horizontal referente ao cesta de ovos
                y: this.position.y, // Posição vertical referente ao cesta de ovos
            },
            width: 200, // Largura da caixa da câmera
            height: 80, // Altura da caixa da câmera
    ...
    checkForHorizontalCanvasCollision() { 
        ...
        ) { // Verifica se o hitbox cesta de ovos colide com as bordas do canvas
            this.velocity.x = 0; // Para a velocidade horizontal cesta de ovos
        }
    }
    ...
    // * Método para atualizar a posição cesta de ovos:
    update() {
        this.updateFrames(); // Atualiza os quadros cesta de ovos
        this.updateHitbox(); // Atualiza a caixa de colisão cesta de ovos
        this.updateCamerabox(); // Atualiza a caixa da câmera cesta de ovos

        // Desenha a camerabox cesta de ovos:
        ...
        // ! Desenha a imagem de fundo cesta de ovos:
        ...
        // ! Desenha o hitbox cesta de ovos:
    ...
    // * Método para atualizar a caixa de colisão cesta de ovos
    updateHitbox() { 
        this.hitbox = {
            position: {
                x: this.position.x + 0, // Posição horizontal referente ao cesta de ovos
                y: this.position.y + 0, // Posição vertical referente ao cesta de ovos
            },
            width: 16, // ! Largura do hitbox
            height: 12, // ! Altura do hitbox
        }
    }
    ...
    checkForHorizontalCollision() {
        for(let i = 0; i < this.collisionBlocks.length; i++) {
            ...
            if (
                collision({
                    object1: this.hitbox, // O hitbox cesta de ovos
                ...
                if (this.velocity.x > 0) {
                    ...
                    const offset = this.hitbox.position.x - this.position.x + this.hitbox.width; // Calcula o deslocamento do hitbox em relação à posição cesta de ovos
                ...
                if (this.velocity.x < 0) {
                    ...
                    const offset = this.hitbox.position.x - this.position.x; // Calcula o deslocamento do hitbox em relação à posição cesta de ovos
    ...
    applyGravity() { 
        this.velocity.y += gravity; // Aumenta a velocidade cesta de ovos com a gravidade
        ...
        for(let i = 0; i < this.collisionBlocks.length; i++) {
            ...
            if (
                ...
                if (this.velocity.y > 0) {
                    this.velocity.y = 0; // Para a velocidade vertical cesta de ovos

                    const offset = this.hitbox.position.y - this.position.y + this.hitbox.height; // Calcula o deslocamento do hitbox em relação à posição cesta de ovos
                ...
                if (this.velocity.y < 0) {
                    ...
                    const offset = this.hitbox.position.y - this.position.y; // Calcula o deslocamento do hitbox em relação à posição cesta de ovos
    ...
    destroy(eggs, index) {
        // Muda a sprite cesta de ovos para "pt-1.png"
